...
object ExtraccionTablaMergedDimension extends App {
  ...
  val path2 = "src/datasets/github/merged"
  ...
  val mergedColDF = githubDF.withColumn("user", col("payload.pull_request.merged_by"))
  ...
  val mergedUniqueDF = mergedColDF.as("merged1").join(mergedColDF.as("merged2"))
    .filter((col("merged1.user_id") === col("merged2.user_id"))&&
      (col("merged1.updated_at").gt(col("merged2.updated_at")) ||
        col("merged1.updated_at") === col("merged2.updated_at")))
    .select(col("merged1.login"), col("merged1.user_id")
      , col("merged1.type"), col("merged1.site_admin"),
      col("merged1.email"), col("merged1.name"), col("merged1.location"),
      col("merged1.hireable"), col("merged1.bio"), col("merged1.company"),
      col("merged1.blog"), col("merged1.twitter_username"),
      col("merged1.created_at"), col("merged1.updated_at"),
      col("merged1.followers"), col("merged1.following"),col("merged1.public_repos"))

  //Eliminando duplicados
  val userDistintcDF = mergedUniqueDF.dropDuplicates("user_id", "login")

  //userDistintcDF.show(10,false)
  userDistintcDF.write.mode(SaveMode.Overwrite).parquet(path2)
  println("--PARQUET MERGED CREADO CON ÉXITO--")
