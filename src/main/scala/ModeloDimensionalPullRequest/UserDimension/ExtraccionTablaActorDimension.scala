...
object ExtraccionTablaActorDimension extends App {
  ...
  val actorDF = githubDF
    ...
    .withColumn(
      "email",
      when(col("user.email").isNull, "Email not available")
        .otherwise(col("user.email"))
    )
    .withColumn(
      "name",
      when(col("user.name").isNull, "Name not available")
        .otherwise(col("user.name"))
    )
    .withColumn(
      "location",
      when(col("user.location").isNull, "Location not available")
        .otherwise(col("user.location"))
    )
    .withColumn(
      "site_admin",
      when(col("user.site_admin").isNull, "Name not available")
        .when(col("user.site_admin") === true, "User is site admin")
        .otherwise("User is not site admin")
    )
    .withColumn(
      "type",
      when(col("user.type").isNull, "Type not available")
        .otherwise(col("user.type"))
    )
    .withColumn(
      "hireable",
      when(col("user.hireable").isNull, "Not available")
        .when(col("user.hireable") === true, "Is hireable")
        .otherwise("Is not hireable")
    )
    .withColumn(
      "bio",
      when(col("user.bio").isNull, "Biography not available")
        .otherwise(col("user.bio"))
    )
    .withColumn(
      "company",
      when(col("user.company").isNull, "Company not available")
        .otherwise(col("user.company"))
    )
    .withColumn(
      "blog",
      when(col("user.blog").isNull, "Blog not available")
        .otherwise(col("user.blog"))
    )
    .withColumn(
      "twitter_username",
      when(
        col("user.twitter_username").isNull,
    ...
    .withColumn(
      "created_at",
      when(col("user.created_at").isNull, "Created at not available")
        .otherwise(col("user.created_at"))
    )
    .withColumn(
      "updated_at",
      when(col("user.updated_at").isNull, "Updated at not available")
        .otherwise(col("user.updated_at"))
    )
    .withColumn(
      "followers",
      when(col("user.followers").isNull, "Followers not available")
        .otherwise(col("user.followers"))
    )
    .withColumn(
      "following",
      when(col("user.following").isNull, "Following not available")
        .otherwise(col("user.following"))
    )
    .withColumn(
      "public_repos",
      when(col("user.public_repos").isNull, "Public repos not available")
        .otherwise(col("user.public_repos"))
    )
    .select(
      col("user.login").as("login"),
      col("user.id").cast(StringType).as("user_id"),
      col("type"),
      col("site_admin"),
      col("email"),
      col("name"),
  ...
  val actorUniqueDF = actorDF
    .as("actor1")
    .join(actorDF.as("actor2"))
    .filter(
      (col("actor1.user_id") === col("actor2.user_id")) &&
    ...
    .select(
      ...
      col("actor1.company"),
      col("actor1.blog"),
      col("actor1.twitter_username"),
      col("actor1.created_at"),
      col("actor1.updated_at"),
      col("actor1.followers"),
      col("actor1.following"),
      col("actor1.public_repos")
    )
