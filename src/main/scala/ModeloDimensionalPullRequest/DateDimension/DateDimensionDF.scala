...
object DateDimensionDF extends App {
  ...
  val dateDimensionTotalDF = dateDimensionDF.withColumn("date_key", monotonically_increasing_id())
    .withColumn("full_date_description",date_format(col("date"),"MMMM dd yyyy")).withColumn("day_of_week", returnDiaUDF(dayofweek(col("date"))))
    .withColumn("calendar_month", returnMonthUDF(month(col("date")))).withColumn("calendar_quarter", quarter(col("date")))
    .withColumn("calendar_year", year(col("date")))
    .withColumn("holiday_indicator", when(
      (month(col("date"))===10 && dayofmonth(col("date"))===31) || (month(col("date"))===12 && dayofmonth(col("date"))===31) ||
        (month(col("date"))=== 1 && dayofmonth(col("date"))===1) || (month(col("date"))===12 && dayofmonth(col("date"))===24) ||
        (month(col("date"))=== 12 && dayofmonth(col("date"))===25), "Holiday"
