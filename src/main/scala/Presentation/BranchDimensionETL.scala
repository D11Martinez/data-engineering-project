...
object BranchDimensionETL {
  ...
  def getDataFrameBranch(
    ...
    val branchDimension = stagingPullRequestDF
      ...
      .select(
        ...
        when(col("description_repo").isNull, "Description not available")
          .otherwise(col("description_repo"))
          .as("description_repo"),
        col("branch.default_branch").as("default_branch_repo"),
        when(col("language_repo").isNull, "Languge not available")
          .otherwise(col("language_repo"))
          .as("language_repo"),
        when(col("license_repo").isNull, "License not available")
          .otherwise(col("license_repo.name"))
          .as("license_repo"),
        when(col("is_forked_repo") === true, "Has been forked")
          .otherwise("Has not been forked")
          .as("is_forked_repo"),
        when(col("archived_repo") === true, "Is archived")
          .otherwise("Is not archived")
          .as("archived_repo"),
        when(col("private_repo") === true, "Private repository")
          .otherwise("Public repository")
          .as("private_repo"),
        when(col("size_repo").isNull, -1)
          .otherwise(col("size_repo"))
          .as("size_repo"),
        col("disabled_repo"),
  ...
  def getDataFrame(
    ...
    val BranchHead =
      getDataFrameBranch(stagingPullRequestDF, "pull_request_head")
